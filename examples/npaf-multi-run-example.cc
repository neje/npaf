...
/*
 * Copyright (c) 2020 University of Belgrade
 *
...
void
RoutingExperiment::WriteToSummaryFile (npaf::RunSummary srs)
{
  std::ofstream out;
...
RoutingExperiment::Run (int argc, char **argv)
{
  ...
  */
  ...
  for (uint32_t i = 0; i<nSources; i++)
    {
      ...
      // Source
      npaf::StatsSourceHelper sourceAppH (transportProtocol, destinationAddress);
      ...
      // Sink 
      npaf::StatsSinkHelper sink (transportProtocol, sinkReceivingAddress);
  ...
  npaf::StatsFlows oneRunStats (m_rngRun, m_csvFileNamePrefix, false, false); // current RngRun, file name, RunSummary to file, EveryPacket to file
  ...
  npaf::RunSummary srs = oneRunStats.Finalize ();
  ...
  while (true)
    {
      ...
      npaf::RunSummary srs = experiment.Run (argc,argv);
